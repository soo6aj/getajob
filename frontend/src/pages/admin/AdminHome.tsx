...
import {
  Users, Briefcase, Building2, Shield, TrendingUp,
  CheckCircle2, UserPlus, FileCheck
} from 'lucide-react';
import {
      ...
      <div className="bg-midnight rounded-2xl p-6 lg:p-8 text-white flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl relative overflow-hidden">
        <div className="relative z-10 space-y-2">
          ...
          <p className="text-gray-300 text-sm max-w-xl">
            Real-time platform metrics, company verification queues, and audit telemetry.
          </p>
        </div>
            ...
            {recentUsers.map(u => (
              <div key={u.id} className="py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold uppercase ${u.role === 'admin' ? 'bg-purple-100 text-purple-700' :
                    u.role === 'recruiter' ? 'bg-teal-100 text-teal-700' : 'bg-blue-100 text-blue-700'
                    }`}>
                    {u.role}
                  </span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${u.status === 'active' ? 'text-green-700 bg-green-50' : 'text-red-700 bg-red-50'
                    }`}>
                    {u.status}
                  </span>
                </div>
